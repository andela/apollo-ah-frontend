...
import { signupType, signUpUser, addError, clearErrors } from '../../actions/signupActions';
...
describe('Testing signup action ', () => {
  it('Should dispatch the success action type when signup is successfull', async (done) => {
    mockAxios.post.mockImplementationOnce(() => Promise.resolve({ data: {data:{ ...success }}}));
  ...
  it('should populate the store with errors for the application', async(done) => {
    const expectedAction = [
      { type: signupType.add_error, data:{ errors: [{ message:"Email is required", field: "email" }] } }
    ];
