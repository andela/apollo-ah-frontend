...
describe('<UserForm />', () => {
  const wrapper = shallow(<UserForm resetState={mockFn} />);
  ...
  it('the default value of the input should be an empty string', () => {
    const formJsx = shallow(<UserForm resetState={mockFn} />);
    expect(formJsx.find('#email').props().value).toBe('');
  });
  it('should change the value of the email input', () => {
    const formJsx = shallow(<UserForm resetState={mockFn} email="blah@gmail.com" />);
  ...
  it('should change the value of the password input', () => {
    const formJsx = shallow(<UserForm resetState={mockFn} password="o987654321" />);
    expect(formJsx.find('#password').props().value).toBe('o987654321');
  });
  it('should call handleChange on email change', () => {
    const formJsx = shallow(<UserForm resetState={mockFn} handleChange={mockFn} />);
  ...
  it('should submit form and call handleLogin', () => {
    ...
    const formJsx = shallow(<UserForm resetState={mockFn} handleLogin={mockHandleLogin} />);
